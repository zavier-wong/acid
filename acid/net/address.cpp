...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
    ...
    if(error) {
        SPDLOG_LOGGER_DEBUG(g_logger, "Address::Lookup getaddress({},{},{}) err={} errstr={}",
...
bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>>& result,int family) {
    ...
    if(err) {
        SPDLOG_LOGGER_DEBUG(g_logger, "Address::GetInterfaceAddresses getifaddrs error={} errstr={}", err, gai_strerror(err));
        return false;
    }
    ...
    try {
        ...
        SPDLOG_LOGGER_ERROR(g_logger, "Address::GetInterfaceAddresses exception");
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char *host_name, uint16_t port) {
    ...
    if(error) {
        SPDLOG_LOGGER_DEBUG(g_logger, "IPAddress::Create({},{}) error={} errstr={}",host_name, port, error, gai_strerror(error));
        return nullptr;
    }
...
IPv4Address::ptr IPv4Address::Create(const char *ip, uint16_t port) {
    ...
    if(result <= 0) {
        SPDLOG_LOGGER_DEBUG(g_logger, "IPv4Address::Create({},{}) rt={} errno={} errstr={}",ip, port, result, errno, gai_strerror(errno));
        return nullptr;
    }
    return rt;
}
...
IPv6Address::ptr IPv6Address::Create(const char *ip, uint16_t port) {
    ...
    if(result <= 0) {
        SPDLOG_LOGGER_DEBUG(g_logger, "IPv6Address::Create({},{}) rt={} errno={} errstr={}",ip, port, result, errno, gai_strerror(errno));
        return nullptr;
    }
    return rt;
}
