...
void Unstable::truncateAndAppend(const std::vector<Entry>& ents) {
    ...
    if (after == offset + (int64_t)entries.size()) {
        ...
        SPDLOG_LOGGER_INFO(g_logger, "replace the unstable entries from index {}", after);
        // 这种情况存储可靠存储的日志还没有被提交，此时新的leader不在认可这些日志，所以替换追加
        offset = after;
        entries = ents;
    } else {
        SPDLOG_LOGGER_INFO(g_logger, "truncate the unstable entries before index {}", after);
...
void Unstable::mustCheckOutOfBounds(int64_t low, int64_t high) const {
    if (low < high) {
        SPDLOG_LOGGER_ERROR(g_logger, "invalid unstable.slice {} > {}", low, high);
    }
    int64_t upper = offset + entries.size();
    if (low < offset || high > upper) {
        SPDLOG_LOGGER_ERROR(g_logger, "unstable.slice[{},{}) out of bound [{},{}]", low, high, offset, upper);
    }
}
}