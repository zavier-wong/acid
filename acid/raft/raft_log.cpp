...
RaftLog::RaftLog(Storage::ptr storage, int64_t maxNextEntsSize /* = NO_LIMIT*/ ) {
    if (!storage) {
        SPDLOG_LOGGER_CRITICAL(g_logger, "storage must not be nil");
        return;
    }
    m_storage = storage;
    ...
    if (first_index < 0 || last_index < 0) {
        SPDLOG_LOGGER_CRITICAL(g_logger, "get index error");
        return;
    }
    ...
    if (matchLog(prevLogIndex, prevLogTerm)) {
        ...
        if (conflict_index < 0 || conflict_index <= m_committed) {
            ...
            if (conflict_index - offset > (int64_t)entries.size()) {
                SPDLOG_LOGGER_CRITICAL(g_logger, "index, {}, is out of range [{}]", conflict_index - offset, entries.size());
            }
...
std::vector<Entry> RaftLog::nextEntries() {
    ...
    if (m_committed + 1 > off) {
        ...
        if (err) {
            SPDLOG_LOGGER_CRITICAL(g_logger, "unexpected error when getting unapplied entries");
        }
        return ents;
    }
    return {};
...
int64_t RaftLog::firstIndex() {
    ...
    if (index < 0) {
        SPDLOG_LOGGER_CRITICAL(g_logger, "unexpected error when getting first index");
    }
    return index;
}
...
int64_t RaftLog::lastIndex() {
    ...
    if (index < 0) {
        SPDLOG_LOGGER_CRITICAL(g_logger, "unexpected error when getting last index");
    }
    return index;
}
...
void RaftLog::commitTo(int64_t tocommit) {
    if (m_committed < tocommit) {
        if (lastIndex() < tocommit) {
            SPDLOG_LOGGER_CRITICAL(g_logger, "tocommit({}) is out of range [lastIndex({})]. Was the raft log corrupted, truncated, or lost?",
               tocommit, lastIndex());
        }
...
int64_t RaftLog::term(int64_t index) {
    ...
    SPDLOG_LOGGER_CRITICAL(g_logger, "unexpected error when getting term");
    return 0;
}
...
std::vector<Entry> RaftLog::allEntries() {
    ...
    SPDLOG_LOGGER_CRITICAL(g_logger, "unexpected error when getting allEntries");
    return {};
}
...
void RaftLog::restore(Snapshot::ptr s) {
    SPDLOG_LOGGER_CRITICAL(g_logger, "log [{}] starts to restore snapshot [index: {}, term: {}]",
...
std::pair<std::vector<Entry>, StorageError> RaftLog::slice(int64_t low, int64_t high, int64_t maxSize) {
    ...
    if (low < m_unstable.offset) {
        ...
        if (err == StorageError::Compacted) {
            ...
            SPDLOG_LOGGER_CRITICAL(g_logger, "entries[{}:{}) is unavailable from storage", low, std::min(high, m_unstable.offset));
        } else if (err) {
            SPDLOG_LOGGER_CRITICAL(g_logger, "unexpected error when slice");
        }
...
StorageError RaftLog::mustCheckOutOfBounds(int64_t low, int64_t high) {
    ...
    if (high > first + length) {
        SPDLOG_LOGGER_CRITICAL(g_logger, "slice[{},{}) out of bound [{},{}]", low, high, first, lastIndex());
    }
    return StorageError::Succeed;
}
