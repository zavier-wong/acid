...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    while (!parser->isFinished()) {
        ...
        if (parser->hasError() || nparse == 0) {
            ACID_LOG_DEBUG(g_logger) << parser->hasError();
            close();
            return nullptr;
        }
        offset = len - nparse;
    ...
    if (parser->isChunked()) {
        do {
            ...
            //ACID_LOG_INFO(g_logger) << data;
            if (len <= 0) {
                ACID_LOG_DEBUG(g_logger) << "parse chunk error, code=" << parser->hasError();
                close();
                return nullptr;
            }
            len += offset;
...
HttpConnection::~HttpConnection() {
    ACID_LOG_DEBUG(g_logger) << "~HttpConnection";
}
...
HttpConnectionPool::Create(const std::string &uri, const std::string &vhost, uint32_t max_size,
    ...
    if(turi) {
        ACID_LOG_ERROR(g_logger) << "invalid uri=" << uri;
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    ...
    if(!ptr) {
        ...
        if(!addr) {
            ACID_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        ...
        if(!sock) {
            ACID_LOG_ERROR(g_logger) << "create sock fail: " << addr->toString();
            return nullptr;
        }
        if(!sock->connect(addr)) {
            ACID_LOG_ERROR(g_logger) << "sock connect fail: " << addr->toString();
            return nullptr;
        }
